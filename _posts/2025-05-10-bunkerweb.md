...
Il faut notamment se prémunir contre les exploits applicatifs, par exemple :
...
En ce qui en est de la gestion des certificats SSL, l’implémentation de Let’s Encrypt est très simple d’utilisation !  
...
Vous avez aussi la possibilité de demander des certificats SSL via **DNS**, permettant ainsi donc l’usage de **Wildcards** (`*.votredomaine.tld`).  
Pour cela :
...
Sur la page d’acceuil, vous retrouverez quelques statistiques essentielles liées à votre instance.  
Dans la barre lattérale gauche se trouve les menus suivants :
...
En haut à gauche, le menu déroulant vous donne accès à la configuration globale de chaque plug-in ; en haut à droite, le bouton **Save** enregistre vos changements.
Ci-dessous, vous trouverez les principaux réglages que j’ai adaptés — présentés par rubrique pour une lecture plus fluide.
...
* `AUTO_LETS_ENCRYPT` : **true** – Pré-remplit les champs Let’s Encrypt lors de la création d’un service.
...
Vous connaissez maintenant les paramètres essentiels ; passons à présent au déploiement d’un service type.

# Déployer un service web pas à pas
...
Dans l’interface BunkerWeb, ouvrez la section **Services** puis cliquez sur **Create new service**. Suivez ensuite les étapes décrites ci-après.
...
Cliquez sur **Save** : BunkerWeb lance le déploiement en arrière-plan (≈ 1 min).
Votre service est maintenant protégé et accessible !

# Conclusion

Et voilà, vous avez maintenant un reverse proxy sécurisé pour toutes vos applications !

J’améliorerais cette documentation au fil du temps en fonction des situations et problèmes que je rencontrerais à l’avenir.
