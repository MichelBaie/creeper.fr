...
#### **⚠ Pour la suite de ce guide, il est nécessaire que vous utilisiez Debian 11> ⚠**
...
⚠ Il est important d'avoir (au moins) une IP Failover commandée et livrée pour la suite du tutoriel. ⚠
...
```bash
sudo su -
apt install wireguard wireguard-tools openresolv bash curl wget -y
```
...
Il peut arriver qu’il y ai des soucis de routage sur le VPS, pour régler votre problème effectuez les commandes suivantes :

```bash
apt-get install arping -y
/usr/sbin/arping -q -c1 -P ipfailover -S ipfailover
```
