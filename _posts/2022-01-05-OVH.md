---
layout: post
title: Avoir des IPV4/IPV6 chez soi. (HMS/OVH + Tunnel WireGuard)
...
tags: [tunnel,ovh,hostmyservers,hms,ipfailover,ipfo,wireguard]
---
...
* Un VPS **[HMS](https://www.hostmyservers.fr/)** ou **[OVH](https://www.ovhcloud.com/fr/vps/)** (Ce sont deux hébergeurs que je certifie compatibles)
* **1 ou plusieurs IP Failover/Additionnelle** (Ce sont les IPs supplémentaires que nous utiliserons sur nos différents clients)

* **Debian 11 ≥**  (Du moment qu'on a un **Kernel Linux 5.10≥**, qui supporte **nativement** **WireGuard**)
* **Une ligne VDSL ou plus** (C'est préférable pour que vos services soient correctement accessibles en dehors de chez vous)

***Justifications :***


* **Aujourd'hui**, j'utilise **[HostMyServers](https://www.hostmyservers.fr/)**, un hébergeur français** qui est beaucoup **plus petit qu'OVH**. C'est le seul autre host que je connaisse qui **propose des ipv4 à tarifs "à vie"**. Il propose lui aussi un **Anti-DDOS** correct, et existe depuis un [**certain temps**](https://www.societe.com/societe/hostmyservers-842789000.html).
...
* **Debian 11≥** **intègre** enfin **WireGuard** via **le kernel linux natif** en **production**, ce qui va nous **simplifier grandement la tâche**.

![](https://korben.info/app/uploads/2020/02/bench.png)

#### **⚠ Il faut obligatoirement que vous utilisiez Debian 11. ⚠** (afin que vous puissiez suivre au mieux cette doc)

## 2 - Tarification

Je vais comparer ici les tarifications d'**OVH** et de **HMS**, il est important de prendre en compte le fait qu'**OVH est une grande société** avec une **certaine renommée internationale**, qui existe depuis très longtemps et **propose** d'**excellents services** (quasiment irréprochables)
Néanmoins, **HostMyServers est une petite société française** qui propose des **tarifs plus attractifs** avec une **fiabilité correcte**.

- Si vous souhaitez une **stabilité/fiabilité irréprochable**, je vous recommande **OVH**
- Si vous souhaitez faire des **économies**, et bénéficier de **rapports** plus **intéressants** **prix/perf**, en étant conscient qu'il peut y avoir de **légères interruptions** de service, je vous recommande **HMS**.

#### **Voici la grille tarifaire d'OVH (au 29/12/2022) :**

| Édition                              | Starter    | Value      | Essential  | Comfort  | Elite    |
| ------------------------------------ | ---------- | ---------- | ---------- | -------- | -------- |
| *Prix mensuel sans engagement (TTC)* | 4,20€      | 6,96€      | 15€        | 28,20€   | 38,09€   |
| *Bande passante*                     | 100 Mbit/s | 250 Mbit/s | 500 Mbit/s | 1 Gbit/s | 2 Gbit/s |
| *vCore(s)*                           | 1          | 1          | 2          | 4        | 8        |
| *Mémoire RAM*                        | 2 Go       | 2 Go       | 4 Go       | 8 Go     | 8 Go     |
| *Stockage SSD*                       | 20 Go      | 40 Go      | 80 Go      | 160 Go   | 160 Go   |

Toutes ces offres **peuvent supporter jusqu'à 16 IP Failovers** (**par VPS**). Elles coûtent **1,80€ TTC** **mensuellement** chacune.

**Si vous choisissez cette option**, prenez **directement** une **IP Failover** depuis votre Espace Client (**Bare Metal Cloud** → **IP** → **Commander des IP additionnelles**), et **sélectionnez bien le bon VPS**. **Quand vous aurez reçu le mail de confirmation**, nous pourrons **continuer**.
...
* Si la **latence est cruciale** pour vous, je vous conseille de prendre un **VPS OVH**.
  Néanmoins, vous pouvez tenter vous même de ping les deux IP ci-dessous et vous faire votre propre avis.
  N'oubliez pas qu'il faut multiplier par 2 cette latence, car une fois le tunnel monté, il faut prendre en compte l'aller-retour.

  **HMS : hms.creepercreep.fr**
  **OVH : ovh.creepercreep.fr** (ce serveur ne m'appartient pas)

* Dans les deux cas, **les VPS ne sont pas des containers LXC bizzare**, et s**upportent donc tout type d'application**.
...
**Ce qui nous intéresse ici**, ce sont les **AllowedIPs**, ce sont les **adresses IP** qui ont été **attribuées** **pour le profil** MaVM. **Modifions celle-ci** en **retirant l'IP locale** (**10.66.66.2/32**) et en **rajoutant l'IP FO HMS/OVH** (par exemple : **92.122.45.218**) .
...
L**'IPV4 Endpoint doit-être celle du VPS HMS/OVH**, puis choisissez une **localisation proche** (pour ma part, Paris).
...
Nous devons également **supprimer la configuration ipv6 inclue nativement par HMS/OVH** :
...
**Si vous utilisez HMS**, il faut **remplacer** **ens3** **par** **eth0**
...
2. Si vous souhaitez que ce soit **effectif de manière globale** à **tous** vos **containers** docker, vous pouvez **modifier le service docker** (`/lib/systemd/system/docker.service`) et rajouter à la fin de **ExecStart** `--mtu=1370` :
   ![](https://cdn.discordapp.com/attachments/926788575293472798/1072587480999665734/Termius_YSuwDpaEKa.png)
